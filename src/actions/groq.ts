...
// "Lascia fare a noi" (AI) dalla setup page: genera macro-argomenti e
// subtopics usando solo titolo/categoria/corso/università dell'esame,
// SENZA un materiale di riferimento. È la variante di analyzeMaterialWithAI
// per il caso in cui l'utente non ha ancora un documento da caricare.
export async function seedExamTopicsFromTitle(examId: string) {
  const rate = await checkRateLimit('analyze_material')
  if (!rate.allowed) return { error: rate.error }
  ...
  const { data: exam } = await supabase
    .from('exams')
    .select('nome_esame, categoria, universita, corso')
    .eq('id', examId)
    .single()

  if (!exam) return { error: 'Esame non trovato' }

  const ex = exam as { nome_esame: string; categoria: string; universita: string; corso: string }

  const categoriaLabel: Record<string, string> = {
    scientifica: 'scientifica (richiede esercizi, problemi e dimostrazioni)',
    mnemonica: 'mnemonica (richiede definizioni, date, formule da ricordare)',
    applicativa: 'applicativa (richiede analisi di casi reali, esercizi su dati concreti)',
  }
  const catLabel = categoriaLabel[ex.categoria] || ex.categoria

  const prompt = `Sei un tutor universitario esperto. Uno studente deve preparare l'esame "${ex.nome_esame}" (materia di tipo ${catLabel}) del corso di laurea "${ex.corso}" presso "${ex.universita}".

Lo studente NON ha ancora un documento di riferimento. Devi estrarre tu i 5-8 macro-argomenti più importanti che tipicamente compongono questo esame universitario, basandoti sul titolo dell'esame, sul tipo di materia e sul corso di laurea.

Per ogni argomento assegna un peso di importanza da 1 a 5 (5 = fondamentale per superare l'esame).

REQUISITI:
- Identifica SOLO argomenti rilevanti per un esame universitario di questa materia
- I pesi devono riflettere l'importanza tipica nel programma
- Se il titolo è generico (es. "Esame 1"), inferisci dal corso di laurea e dal tipo di materia
...
RISPONDI SOLO con un JSON valido nel formato:
{
  ...
  "sommario": "Breve descrizione del piano di studio generato (max 200 caratteri)"
}`
  ...
  try {
    ...
    const parsed = safeParseJSON<{ argomenti: GroqTopic[], sommario: string }>(text)
    if (!parsed || !Array.isArray(parsed.argomenti) || parsed.argomenti.length === 0) {
      return {
        success: false,
        degraded: true,
        error: 'AI ha risposto in un formato non leggibile. Riprova o aggiungi argomenti manualmente.',
        topics: [],
        sommario: ''
      }
    }

    // Sostituiamo i topic esistenti (subtopics e competences cascano via FK)
    await supabase.from('topics').delete().eq('exam_id', examId)
    ...
    if (insertedTopics && insertedTopics.length > 0) {
      ...
      const settled = await Promise.allSettled(
        topics.map(t => generateSubtopicsForTopic(t.id, t.nome_argomento))
      )
      failedCount = settled.filter(s => s.status === 'rejected').length
...
export async function generateQuizFromMaterial(
  ...
  // Carico topic + esame in parallelo per contestualizzare il prompt.
  // Senza questo il prompt conosce solo il nome del macro-argomento e
  // produce domande generiche non legate al corso specifico dello
  // studente.
  const [{ data: topic }, { data: exam }] = await Promise.all([
    supabase.from('topics').select('nome_argomento, peso').eq('id', topicId).single(),
    supabase.from('exams').select('nome_esame, categoria').eq('id', examId).single(),
  ])

  if (!topic) return { error: 'Argomento non trovato' }
  if (!exam) return { error: 'Esame non trovato' }

  // Subtopic opzionale: se passato, usiamo titolo+descrizione per
  // rendere le domande specifiche del micro-argomento, non solo del
  // macro-argomento.
  let subtopicContext = ''
  if (subtopicId) {
    const { data: sub } = await supabase
      .from('subtopics')
      .select('titolo, descrizione, tipo')
      .eq('id', subtopicId)
      .single()
    if (sub) {
      const s = sub as { titolo: string; descrizione: string | null; tipo: string }
      subtopicContext = `\nMicro-argomento specifico: "${s.titolo}"${s.descrizione ? ` (${s.descrizione})` : ''}`
    }
  }

  // Cast perché l'inferenza supabase-js v2 può collassare a `never`.
  const t = topic as { nome_argomento: string; peso: number }
  const ex = exam as { nome_esame: string; categoria: string }

  // Istruzioni specifiche per livello di difficoltà. Manteniamo il JSON
  // schema invariato per non rompere la UI (DailyPlan.tsx si aspetta
  // { testo, opzioni[4], risposta_corretta, spiegazione }).
  const difficultyInstructions: Record<typeof difficulty, string> = {
    easy: '- Domande di DEFINIZIONE e RICONOSCIMENTO di concetti base\n- Distrattori palesemente errati (es. definizioni sbagliate in modo ovvio)\n- Una sola applicazione diretta per ancorare',
    medium: '- Mix equilibrato di teoria (3 domande) e applicazione (2 domande)\n- Distrattori ragionevoli ma riconoscibili se hai studiato',
    hard: '- Applicazione MULTI-STEP, scenari realistici d\'esame\n- Distrattori PLAUSIBILI e trappole concettuali (es. confondono definizioni simili, formule collegate)\n- Almeno una domanda che richiede ragionamento non solo memoria',
  }

  const prompt = `Sei un professore universitario. Crea un quiz per l'esame "${ex.nome_esame}" (materia di tipo ${ex.categoria}).

Argomento: "${t.nome_argomento}"${subtopicContext}

DIFFICOLTA: ${difficulty.toUpperCase()}
${difficultyInstructions[difficulty]}

REQUISITI:
- 5 domande a risposta multipla in italiano
- Per ogni domanda: testo della domanda, 4 opzioni (A/B/C/D), indice della risposta corretta (0-3), spiegazione breve
- Le opzioni devono essere plausibili per lo studente che NON ha studiato; una sola è corretta
- Le domande devono essere realistiche per un esame universitario italiano di questa materia
- Varia i tipi di domanda (non fare 5 domande tutte sulla stessa definizione)
