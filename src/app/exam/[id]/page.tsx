...
type TabName = 'piano' | 'materiali' | 'calendario'
...
export default function ExamPage() {
  ...
  useEffect(() => {
    // Leggo il tab iniziale dalla query string per supportare
    // /exam/[id]?tab=materiali (usato dal bottone "Vai ai materiali"
    // della setup page). window.location evita il vincolo Suspense di
    // useSearchParams.
    if (typeof window !== 'undefined') {
      const tab = new URLSearchParams(window.location.search).get('tab')
      if (tab === 'piano' || tab === 'materiali' || tab === 'calendario') {
        setInitialTab(tab)
      }
    }
  }, [])
