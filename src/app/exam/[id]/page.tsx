      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-600 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/dashboard" className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-500 dark:text-slate-400 transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div>
              <h1 className="font-bold text-xl text-slate-900 dark:text-white">{exam.nome_esame}</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">{exam.universita} • {exam.corso}</p>
            </div>
          </div>
          <form action={signOut}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <div className="card py-4">
            <div className="flex items-center gap-2 text-coach-600 dark:text-coach-400 text-sm mb-1">
              <Calendar className="w-4 h-4" /> Data
            </div>
            <p className="font-semibold text-coach-600 dark:text-coach-400 text-sm">{formatDate(exam.data_esame)}</p>
          </div>
          <div className="card py-4">
            <div className="flex items-center gap-2 text-coach-600 dark:text-coach-400 text-sm mb-1">
              <Target className="w-4 h-4" /> Obiettivo
            </div>
            <p className="font-semibold text-coach-600 dark:text-coach-400 text-sm">{exam.voto_obiettivo}/30</p>
          </div>
          <div className="card py-4">
            <div className="flex items-center gap-2 text-coach-600 dark:text-coach-400 text-sm mb-1">
              <Clock className="w-4 h-4" /> Ore/giorno
            </div>
            <p className="font-semibold text-coach-600 dark:text-coach-400 text-sm">{exam.ore_giorno}h</p>
          </div>
          <div className="card py-4">
            <div className="flex items-center gap-2 text-coach-600 dark:text-coach-400 text-sm mb-1">
              <TrendingUp className="w-4 h-4" /> Preparazione
            </div>
            <p className={`font-semibold text-coach-600 dark:text-coach-400 text-sm ${exam.preparazione_percentuale < 30 ? 'text-red-600 dark:text-red-400' : exam.preparazione_percentuale < 70 ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400'}`}>{exam.preparazione_percentuale}%</p>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="card">
              ...
              <div className="space-y-1">
                {topics.map((topic) => (
                  <div key={topic.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-600 last:border-0">
                    <span className="text-sm text-slate-700 dark:text-slate-300">{topic.nome_argomento}</span>
            ...
            <div className="bg-slate-900 rounded-2xl p-5 text-white">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 bg-coach-500 rounded-lg flex items-center justify-center">
                  <GraduationCap className="w-4 h-4 text-white" />
                </div>
                <span className="font-bold text-xs text-coach-300 uppercase tracking-wider">Coach</span>
              </div>
              <p className="text-sm text-coach-600 dark:text-coach-400 leading-relaxed">
